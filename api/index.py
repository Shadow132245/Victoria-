...
def check_content_with_ai(text):
    ...
    try:
        ...
        if "مرفوض" in chat_completion.choices[0].message.content.strip(): return False
        return True
    except: return True

# 🔥 الدالة الجديدة الذكية لجلب اسم الرتبة
def get_user_server_data(user_id):
    ...
    try:
        ...
        if member_resp.status_code == 200:
            ...
            # جلب أسماء كل رتب السيرفر لمعرفة أعلى رتبة للعضو
            ...
            if roles_resp.status_code == 200:
                ...
                # ترتيب الرتب من الأعلى للأسفل
                all_roles.sort(key=lambda x: x['position'], reverse=True)
                
                # استخراج اسم أعلى رتبة يمتلكها العضو
...
def add_contribution():
    ...
    if db_collection is not None:
        db_collection.insert_one({
            ...
            "type": data['type'], "content": data['content'], "upvotes": 0, "status": post_status, 
            "is_admin": is_admin, "role_name": role_name
        })
        msg = "تم النشر بنجاح!" if is_admin else "تم إرسال مساهمتك وهي الآن قيد المراجعة من الإدارة."
...
def get_contributions():
    ...
    for item in cursor:
        results.append({
            ...
            'type': item.get('type'), 'content': item.get('content'), 'upvotes': item.get('upvotes', 0),
            'status': item.get('status', 'approved'), 'is_admin': item.get('is_admin', False), 'role_name': item.get('role_name', '')
        })
...
def approve_post(post_id):
    ...
    if db_collection is not None: db_collection.update_one({'_id': ObjectId(post_id)}, {'$set': {'status': 'approved'}})
    return jsonify({'status': 'success'}), 200
